...
import com.hk.daos.TuDao;
import com.hk.dtos.TuDto;


@WebServlet("/TuController.do")
...
//------------조회수관련 코드 종료---------
			...
			List<TuDto> list=dao.getAllList();
			request.setAttribute("list", list);
//			request.getRequestDispatcher("boardlist.jsp").forward(request, response);
			dispatch("tuboardlist.jsp", request, response);
			...
			if(isS) {
				jsForward("글을 삭제합니다", "TuController.do?command=boardlist", response);
			}else {
			...
			boolean isS=dao.insertBoard(new TuDto(id,title,content));
			
			if(isS) {
				response.sendRedirect("TuController.do?command=boardlist");
			}else {
...
//			}
			...
			boolean isS=dao.replyBoard(new TuDto(seq,id,title,content));
			if(isS) {
				response.sendRedirect("TuController.do?command=boardlist");
			}else {
			...
			boolean isS=dao.updateBoard(new TuDto(seq,title,content));
			if(isS) {
				//detailboard.jsp:응답할 페이지를 먼저 확인해본다.
				response.sendRedirect("TuController.do?command=detailboard&seq="+seq);
			}else {
				response.sendRedirect("error.jsp?msg="+URLEncoder.encode("수정실패", "utf-8"));
			}
		}else if(command.equals("contentAjax")) {
			int seq=Integer.parseInt(request.getParameter("tseq"));
			
			TuDto dto=dao.getTuBoard(seq);//seq에 대한 글상세내용 조회--> dto담음
...
//-----------여러값을 json으로 응답할때 코드 예시: map-->json
			Date regdate=dto.getTregdate();//date 값 가져오기
			...
			Map<String, TuDto>map=new HashMap<>();
			map.put("dto", dto);
