...
<%@page import="com.hk.dtos.SFileDto"%>
...
<%
   ...
   LoginDto ldto=(LoginDto)session.getAttribute("ldto");
	SFileDto fdto = (SFileDto)request.getAttribute("fdto");
         ...
         <tr>
				...
				<td>
					<%
						if(fdto==null){
							%>첨부된 파일 없음<%
						}else if(fdto.getSseq()==dto.getSseq()){
							%>
							<a href="StuController.do?command=download&sseq=<%=fdto.getSseq()%>"><%=fdto.getSorigin_fname()%></a>
							<%
						}else{
							%>파일을 불러오지 못함<%
						}
